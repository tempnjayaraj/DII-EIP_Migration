...
// Motor Stall delay before shutting off the motor
// The Short period is to be used when testing a fully assembled device
// The Long period is to be used when testing control buttons 

#define DEVICE_STALL_SHUTOFF_SHORT		 60		//  6 Seconds
#define DEVICE_STALL_SHUTOFF_LONG		200		// 20 Seconds

// The current sensor circuit cannot read currents below 210mA so zero current
// is scaled to report 210mA (33 ADC Counts).

#define PORT_MIN_REPORTED_CURRENT	210
#define PORT_MIN_CURRENT_ADC		33
...
typedef struct 
{
	...
	SnBool bHandButtonState;
	SnWord usSafeMode;
	SnWord usVelocity;		// Current Velocity
    SnWord usPrevVelocity;
	SnWord usActualVelocity;
	SnWord usActualVelocityAbs;
	SnSQByte qbActualVelocityAbs;
	SnWord wActualVelocityCount;
	SnSWord sMotorCurrent;
	SnSWord sMotorCurrentAdc;
	SnSWord sMotorCurrentAdcCount;
	SnSWord sMotorTac;
	SnWord wPeriod;		// Current period for Oscillate Mode 2
	SnWord wPrevPeriod;
    SnWord usBladeId;		// Blade Id
    ...
    SnQByte qMotorRunTime;
    SnQByte qShaverPowerOnCount;
    SnWord wWindowLockMagnetPositionValue;
    SnWord wWindowLockMagnetPositionValueMax;
} SN_PORT_STATUS;
...
public:
	...
	SnSWord GetMotorCurrent(void);
	SnSWord GetMotorTac(void);
...
private:
	...
	SnWord				m_usMotorStallThreshold;
	...
	SlidingWindowFilter m_cVelocity;
	SlidingWindowFilter m_cVelocitySlow;
	SlidingWindowFilter m_cVelocityFast;
	SlidingWindowFilter m_cMotorCurrent;
};
