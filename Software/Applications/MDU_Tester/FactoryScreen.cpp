...
SnBool CFactoryScreen::SerialNumberTest(void)
{
    ...
    if (pcSerialNumber[0] == 0) {
        CSerialNumberPopUp dlg(m_pControl, 0);
	    dlg.DoModal();
    }
...
SnBool CFactoryScreen::CanTest(void)
{
    StartNewTest(TEXT("CAN...\t\t"));

	do {
		SnBool yCanTestFailed = !m_pControl->CanTest();

		if (yCanTestFailed) {
			ReportTestFailed(TEXT("CAN Test Failed\r"));
		} else {
			ReportTestPassed();
		}
	} while (m_yRepeatTest);
...
SnBool CFactoryScreen::HandpieceTest(int iPort)
{
	...
	do {
        ...
        NewInstructions(TEXT("Plug a PowerMax or PowerMax Elite MDU into the "));
		AddInstructions(csConnector);
		AddInstructions(TEXT(". "));
		...
		wType = (iPort == HAND_PORTA) ? m_pControl->GetPortAType() : m_pControl->GetPortBType();
		if (wType != TYPE_MDU_POWERMINI  && wType != TYPE_MDU_POWERMINI_CTL && wType != TYPE_MDU_POWERMINI_BF) {
			qHandpieceError |= HAND_TEST_RS485;
		}
	...
	do {
		...
		do {
            ...
            if (wPressed == 0) {
                ...
                case 0:
                    if (wState == 0) {
		                NewInstructions(TEXT("Press and release the LOGIC 1 button.\r"));
		                AddInstructions(TEXT("Press the FAIL button if the LOGIC 1 button has been pressed."));
                        wState++;
                    }
                    break;
			    case FOOT_TEST_LOGIC1:
				    if (wState == 1 && !(wValid & FOOT_TEST_LOGIC1)) {
					    wValid |= wPrevPressed;
			    ...
			    case FOOT_TEST_LOGIC0:
				    if (wState == 2 && !(wValid & FOOT_TEST_LOGIC0)) {
					    wValid |= wPrevPressed;
                    ...
                    break;
			    case FOOT_TEST_FWD:
				    if (wState == 3 && !(wValid & FOOT_TEST_FWD)) {
					    wValid |= wPrevPressed;
				    ...
				    break;
			    case FOOT_TEST_REV:
				    if (wState == 4 && !(wValid & FOOT_TEST_REV)) {
					    wValid |= wPrevPressed;
				    ...
				    break;
			    case FOOT_TEST_OSC:
				    if (wState == 5 && !(wValid & FOOT_TEST_OSC)) {
					    wValid |= wPrevPressed;
		                NewInstructions(TEXT("Press and release the SPEED UP button.\r"));
		                AddInstructions(TEXT("Press the FAIL button if the SPEED UP button has been pressed."));
                        wState++;
				    }
				    break;
			    case FOOT_TEST_LOGIC2:
				    if (wState == 6 && !(wValid & FOOT_TEST_LOGIC2)) {
					    wValid |= wPrevPressed;
                        wState++;
				    }
				    break;
		...
		} while (!m_yKillThreads && wState != 7 && !m_yRightPressed);
		if (m_yKillThreads) {
			return FALSE;
		}
		...
		NewInstructions(TEXT("Plug the DYONICS POWER Low Profile Variable Footswitch (7205397) into the "));
		AddInstructions(csConnector);
		AddInstructions(TEXT(". "));
...
#if !ALWAYS_PASS
		if (m_pControl->GetFootPedalType() != TYPE_ANALOG_FOOTPEDAL) {
