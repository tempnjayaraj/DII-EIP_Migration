...
// Function:	WriteWindowLockPostionValue
// Purpose:		Writes the windowlock position value to the Motor Control board 
//
// Parameters:	None
//
// Return:		TRUE indicates Success, FALSE indicates Failure
...
/*SnBool CPort::WriteWindowLockPositionValue(void)
{
    SnBool bStatus = FALSE;
	// Write the velocity
	bStatus = m_pControl->WriteDsp(m_tOffsetAddr.qWindowLockPositionVal, 1, &m_tPortStatus.qWindowLockPositionValue);
	if(!bStatus)
		return FALSE;
...
SnBool CPort::ReadPortStatus(void)
{
	...
	case LOGIC_ID_RS485:
		...
		{	
			...
			case RS485_TYPE_MDU_RELIANT_BF:
				// Reliant BF MDU with Hand Controls
                usType = TYPE_MDU_RELIANT_BF;
 				if (m_tPortStatus.usPrevType != TYPE_MDU_RELIANT_BF) {
                    bStatus = m_pControl->SendSerialRequestMotorTable(m_ucPort);
                    if (bStatus) {
                        yRequestSerialNumber = TRUE;
                    } else {
			...
			case RS485_TYPE_MDU_POWERMINI_BF:
				// Small Joint POWERMINI MDU with Hand Controls
                usType = TYPE_MDU_POWERMINI_BF;
				yRequestSerialNumber = TRUE;
				break;
			default:
				break;
		...
		case TYPE_MDU_RELIANT_BF:
			...
			bHallbusHandControls = FALSE;
			bSerialHandControls = ((usType == TYPE_MDU_RELIANT_CTL) || (usType == TYPE_MDU_RELIANT_BF));
			bMduFound = TRUE;
			m_tPortStatus.yForceOscMode1 = FALSE;
			break;
		case TYPE_MDU_POWERMINI:
		...
		case TYPE_MDU_POWERMINI_BF:
			...
			bHallbusHandControls = FALSE;
			bSerialHandControls = ((usType == TYPE_MDU_POWERMINI_CTL) || (usType == TYPE_MDU_POWERMINI_BF));
			bMduFound = TRUE;
			m_tPortStatus.yForceOscMode1 = FALSE;
			break;
		default:
    ...
    default:
		...
		strcpy(m_tPortStatus.pcSerialNumber, "");				// Erase the serial number of the removed handpiece
        break;
	}	// End switch (usLogic)
	...
	if( bMduFound) 
	{
		...
		else if (( usType == TYPE_MDU_POWERMINI)|| ( usType == TYPE_MDU_POWERMINI_CTL) || ( usType == TYPE_MDU_POWERMINI_BF))
		{
			// If there is no Hand Control Timeout:
			//   - Decode if no FootPedal device is found or 
			//     a FootPedal is found but the Hand Control Overide is disabled.
				...
				usType != m_tPortStatus.usPrevType) && !m_tPortStatus.bHandCtrlTimeout)
			{
               ...
               if ((usType == TYPE_MDU_POWERMINI_CTL)|| (usType == TYPE_MDU_POWERMINI_BF))
                {
		...
		else if (( usType == TYPE_MDU_RELIANT)||( usType == TYPE_MDU_RELIANT_CTL) || ( usType == TYPE_MDU_RELIANT_BF))
		{
			// If there is no Hand Control Timeout:
			//   - Decode if no FootPedal device is found or 
			//     a FootPedal is found but the Hand Control Overide is disabled.
				...
				usType != m_tPortStatus.usPrevType) && !m_tPortStatus.bHandCtrlTimeout)
			{
                ...
                if (( usType == TYPE_MDU_RELIANT_CTL) || ( usType == TYPE_MDU_RELIANT_BF))
                {
                    ...
                    //Get WindowLock Magnet Position
                    newPwRequests[0] = HAND_PORT_CMD(m_ucPort, SERIAL_CMD_REQ_8);
                    newPwRequests[1] = 0;
                    ...
                    {
                        static SnQByte lMaxValue = 0;
                        m_tPortStatus.wWindowLockMagnetPositionValue = (newPwRequests[0] & 0x3FF);
                        
                        if(lMaxValue <= m_tPortStatus.wWindowLockMagnetPositionValue)
                        {
                            lMaxValue = m_tPortStatus.wWindowLockMagnetPositionValue;
                            m_tPortStatus.wWindowLockMagnetPositionValueMax = m_tPortStatus.wWindowLockMagnetPositionValue;
                        }
                    }
