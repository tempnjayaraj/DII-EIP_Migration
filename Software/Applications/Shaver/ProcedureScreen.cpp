...
BOOL CProcedureScreen::OnInitDialog() 
{
    ...
    CreateSetSpeedBitmaps(mem.m_Font50Bold);
...
SnBool CProcedureScreen::CreateSetSpeedBitmaps(CFont *pFont50Bold)
{
    const TCHAR *pwDigits = TEXT("0123456789");
    RECT tRect = {0, 0, 36, 83};
    CBitmap * pOldBitmap;
    CSharedMemory mem;
    CFont * pOldFont;
    SnQByte qDigit;
    ...
    pOldFont = hMemCDC.SelectObject( pFont50Bold);
    ...
    for (qDigit = 0; qDigit < 10; qDigit++) {
        m_hIdleSetSpeedDigit[qDigit].CreateCompatibleBitmap(CSnHelp::m_hDisplayCDC, 36, 83);
    ...
    for (qDigit = 0; qDigit < 10; qDigit++) {
        m_hRunSetSpeedDigit[qDigit].CreateCompatibleBitmap(CSnHelp::m_hDisplayCDC, 36, 83);
