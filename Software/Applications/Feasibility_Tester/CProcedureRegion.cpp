...
void CProcedureRegion::RedrawNonStatics(void)
{
	...
	case PORT_TYPE_TOOL:
        ...
        if(m_tPortStatus.usType == TYPE_MDU_RELIANT_CTL)
            DrawHandpieceCountButtonPortA(TRUE);
        else if(m_tPortStatus.usType == TYPE_MDU_RELIANT_CTL)
...
{
    CHandpieceCountPopUpPortA dlg(m_pControl);
    dlg.DoModal();
}

void CProcedureRegion::OnButtonHandpieceCountPressedPortB()
{
    CHandpieceCountPopUpPortB dlg(m_pControl);
    dlg.DoModal();
}
