...
// KW - Use the Hardware Blt more often (was 1000)
#define NO_HARDWARE_BLT_THRESOLD	(500)
//-----------------------------------------------------------------------------
//! \brief		Constructor
//!
...
SCODE LCDC63hw::DrawBlt(GPEBltParms *pBltParms)
{
	...
	if (!m_pPrimarySurface->Rotate())
	{
		...
		if ( pBltParms->pDst->InVideoMemory() && ((pBltParms->bltFlags & (BLT_TRANSPARENT | BLT_STRETCH | BLT_ALPHABLEND)) == 0) )
		{
            ...
            // KW - Accelerate fill of 0x0000 and 0xFFFF
            case 0xF0F0:
                if (pBltParms->solidColor == 0)
                    Rop4 = 0x0000;
                else if (pBltParms->solidColor == 0xFFFF)
                    Rop4 = 0xFFFF;
                else
                    break;
